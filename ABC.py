...
class ABC:
    ...
    def __init__(_self, conf, datapath):
        _self.data = pd.read_excel(datapath)
        _self.data_features = _self.data.columns
        _self.num_features_selected = 10
    ...
    def init(_self, index):
        if (not (_self.stopping_condition())):
            feature_selected = []
            for i in range(_self.num_features_selected):
                random_num = random.random()
                if _self.data_features[int(random_num * len(_self.data_features))] not in feature_selected:
                    feature_selected.append(_self.data_features[int(random_num) * len(_self.data_features)])
            _self.solution = pd.DataFrame(feature_selected)
